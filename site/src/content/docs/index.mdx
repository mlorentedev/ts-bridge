...
EOF
chmod 600 .env   # an auth key is a credential; the default umask is not 600

# Run
./ts-bridge connect
```

On a shared or hardened host, skip the plaintext step: put the key in its own file and pass
`--auth-key-file ~/.config/ts-bridge/authkey` (mode `600`) instead of setting `TS_AUTHKEY`. See
[Getting started](./getting-started/) for both forms.
