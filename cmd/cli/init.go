...
func buildEnvConfigContent(f initFlags) string {
	...
	sb.WriteString("#\n")
	sb.WriteString("# Note: Keys in .env/environment are visible to child processes.\n")
	sb.WriteString("# For hardened setups, consider omitting TS_AUTHKEY and passing --auth-key-file /path/to/key\n")
...
func printNextSteps(f initFlags) {
	...
	fmt.Println("  To avoid storing keys in environment variables, use --auth-key-file:")
	fmt.Println("  ts-bridge connect --auth-key-file /path/to/keyfile")
	fmt.Println()
}
